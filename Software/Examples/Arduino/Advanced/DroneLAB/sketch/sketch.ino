/*
 AIDA Drone LAB Synth Sketch
 	
 This sketch is an example of a Drone LAB Synth
 with Aida DSP. 
 Info here:
...
void setup()
{
  ...
  Serial.begin(115200);
  clearAndHome();
  Serial.println("Aida DSP control with ARDUINO"); // Welcome message
  Serial.print("0x");
  ...
  spettacolino();
  MasterVolumeMono(DEVICE_ADDR_7bit, VolDistAddr, 0.00);    // With DAC in mute, set volume to 0
  delay(1);   
...
void loop()
{
  ...
  if(time-prevtime >= 1000)  // Here we manage control interface every second
  {
    ...
    Serial.println("*      AIDA DroneLAB Synth     *");
    Serial.println("********************************");
    ...
    if(mute == OFF)
    {
      if(submenu == OFF)
      {
        ...
        delay(1);
        square_source(DEVICE_ADDR_7bit, Square2Addr, frequency);
        delay(1);
        square_source(DEVICE_ADDR_7bit, Square3Addr, frequency);
        delay(1);
        square_source(DEVICE_ADDR_7bit, Square4Addr, frequency);
        delay(1);
        ...
        //MasterVolumeMono(DEVICE_ADDR_7bit, VolDistAddr, volume);       
      }
      else
      {
        //MasterVolumeMono(DEVICE_ADDR_7bit, VolDistAddr, volume);
    ...
    {
      //MasterVolumeMono(DEVICE_ADDR_7bit, VolDistAddr, 0.00);
