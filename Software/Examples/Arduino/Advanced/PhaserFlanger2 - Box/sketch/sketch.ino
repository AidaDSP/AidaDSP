...
void loop()
{
  ...
  { 
    ...
    case 7: // Feedback
      ...
      feedback = (uint8_t)processencoder(0, 100, feedbackpulses);
...
void setFeedback(uint8_t percent)
{
  static uint8_t oldpercent = 0;
  float value = 0.00;
  
  if(fx_counter==2) // Flanger
  {
    if(oldpercent != percent)
    {
      if(percent>100)
      ...
      // MIX
      AIDA_SAFELOAD_WRITE_VALUE(DEVICE_ADDR_7bit, FlangerFlangerFeedbackAddr, false, value);  // Feedback	 
      AIDA_SAFELOAD_WRITE_VALUE(DEVICE_ADDR_7bit, FlangerFlangerFeedbackAddr+1, true, 1.0);   // Input
    }
  }
}
