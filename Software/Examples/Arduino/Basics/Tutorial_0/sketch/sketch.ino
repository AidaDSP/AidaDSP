/*
 AIDA Tutorial_0 Sketch
 	
 This sketch controls three LEDS on Aida DSP Board using the 
...
void loop()
{
  ...
  if(timec-prevtimec >= 250)  // Here we manage control interface every 250ms
  {
    ...
    Serial.println(F("*    AIDA Tutorial_0 Sketch    *"));
    Serial.println(F("********************************"));
    ...
    if(submenu==ON)
    {
      ...
      case 1:
        ...
        dc_source(DEVICE_ADDR_7bit, DC1Addr, 0.0); // 
        dc_source(DEVICE_ADDR_7bit, DC2Addr, 1.0);
        dc_source(DEVICE_ADDR_7bit, DC3Addr, 1.0);
        break;
      case 2:
        Serial.println(F(" LED2 On..."));
        dc_source(DEVICE_ADDR_7bit, DC1Addr, 1.0);
        dc_source(DEVICE_ADDR_7bit, DC2Addr, 0.0);
        dc_source(DEVICE_ADDR_7bit, DC3Addr, 1.0);
        break;
      case 3:
        ...
        dc_source(DEVICE_ADDR_7bit, DC1Addr, 1.0);
        dc_source(DEVICE_ADDR_7bit, DC2Addr, 1.0);
        dc_source(DEVICE_ADDR_7bit, DC3Addr, 0.0);
        break;
      case 4:
