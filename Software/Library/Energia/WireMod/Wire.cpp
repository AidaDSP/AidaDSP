 ...
 ***********************************************************************
  ...
  Modified 13/10/2014 by Massimo Pennazio and Gaetano Grano for AidaDSP integration
 */
...
#define TX_BUFFER_FULL2		(((txWriteIndex + 2) % BUFFER_LENGTH) == txReadIndex)
...
size_t TwoWire::writeBlock(uint8_t *data, size_t quantity, uint16_t internalAddress)
{
	...
	txBuffer[txWriteIndex] = MSB;
	txWriteIndex = (txWriteIndex + 1) % BUFFER_LENGTH;
	txBuffer[txWriteIndex] = LSB;
	txWriteIndex = (txWriteIndex + 1) % BUFFER_LENGTH;
